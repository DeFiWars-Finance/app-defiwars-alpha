...
class Store extends React.Component {
  ...
  async war() {
    if (await this.checkNet()) {
      ...
      const dwarfContract = new web3.eth.Contract(dwarfABI, dwarfAddress)
      try {
        let result = await dwarfContract.methods.war().send({ from: accountAddress })
      } catch (error) {
        this.setReady(false)
      }
    }
  ...
  async checkNFT() {
    ...
    console.log(balance)
    const dwarfContract = new web3.eth.Contract(dwarfABI, dwarfAddress)
    let havenft = await dwarfContract.methods.exists(accountAddress).call({ from: accountAddress })
    console.log(havenft)
    if (havenft) {
      let isInWar = await dwarfContract.methods.isInWar(accountAddress).call({ from: accountAddress })
  ...
  async checkMarket() {
    ...
    try {
      ...
      let opened = await dwarfContract.methods.isOpened(accountAddress).call({ from: accountAddress })
      if (opened) {
        console.log(opened)
      }
      ...
      let staked = await dwarfContract.methods.staked(accountAddress).call({ from: accountAddress })
      if (staked) {
        console.log(staked)
        ...
        let canClaim = await dwarfContract.methods.canClaim(accountAddress).call({ from: accountAddress })
      }
