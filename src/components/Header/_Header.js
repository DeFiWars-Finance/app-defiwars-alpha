...
class Header extends React.Component {
  ...
  addAddress() {
    const address = store.getStore('accountAddress');
    this.setState({
  ...
  checkReady() {
    let inProcess = store.getStore('inProcess');
    let loggedin = store.getStore('loggedin');
  ...
  checkNFTs() {
    let haveNFT = store.getStore('haveNFT');
    this.setState({ haveNFT:haveNFT })

    let isInWar = store.getStore('isInWar');
    this.setState({ isInWar:isInWar })

    let netId = store.getStore('netId');
  ...
  changeStatus() {
    ...
    let haveNFT = store.getStore('haveNFT');
    let isInWar = store.getStore('isInWar');
    let mainNetId = store.getStore('mainNetId');
    let netId = store.getStore('netId');
    if (!inProcess) {
