...
class Header extends React.Component {
  constructor(props) {
    ...
    const netId = store.getStore('netId');
  ...
  componentWillMount() {
    ...
    emitter.on('loggedin', this.checkReady);
  ...
  componentWillUnmount() {
    ...
    emitter.removeListener('loggedin', this.checkReady);
  ...
  render() {
    const {
      isInWar, haveNFT, netId, inProcess, mainNetId, loggedin
      } = this.state;
    var connectButton = 'Connect <br />Wallet';
    store.connect();
    if (inProcess) {
      connectButton = 'processing...';
      }
      if (netId !== mainNetId) {
        ...
        if (!loggedin) {
          ...
          if (!haveNFT) {
            connectButton = 'Mint my WAR';
            store.mint();
            }
            while (!isInWar) {
              connectButton = 'Provide Liquidity <br />on PoLPs';
              this.props.history.push('/Liquidity_Pools');
              connectButton = 'Enable <br />Warzone';
              store.war();
              connectButton = 'Scheduled <br />Warfare';
              }
              this.props.history.push('/NFA_Collections');
              connectButton = 'Mint my PEACE';
              store.peace();
              this.props.history.push('/NFA_Market');
        ...
        <header className={appStyle.flexauto}>
          <div className={style.logo}>
            ...
            <div className={style.connectWallet}>
              <a href='undefined' onClick={this.changeStatus}>
